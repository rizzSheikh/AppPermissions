package com.droids.apppermission;
...
/**
 * Created by Rizwan Sheikh on 04-Aug-20.
 */
...
public class AppPermissions {
    ...
    static void log(String message) {
        if (loggingEnabled) Log.d("AppPermissions", message);
    }
    ...
    public static void check(final Context context, String[] permissions, String rationale,
                             Options options, final AppPermissionHandler handler) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            ...
            if (allPermissionProvided) {
                handler.onGranted();
                log("Permission(s) " + (AppPermissionsActivity.permissionHandler == null ?
                        "already granted." : "just granted from settings."));
                AppPermissionsActivity.permissionHandler = null;

            } else {
                AppPermissionsActivity.permissionHandler = handler;
                ArrayList<String> permissionsList = new ArrayList<>(permissionsSet);

                Intent intent = new Intent(context, AppPermissionsActivity.class)
                        .putExtra(AppPermissionsActivity.EXTRA_PERMISSIONS, permissionsList)
                        .putExtra(AppPermissionsActivity.EXTRA_RATIONALE, rationale)
                        .putExtra(AppPermissionsActivity.EXTRA_OPTIONS, options);
    ...
    public static class Options implements Serializable {
         ...
         * the method {@link AppPermissionHandler#onDenied(Context, ArrayList)} will be invoked
